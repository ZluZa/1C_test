			...
			if (matches != null && matches.Count > 0) {
				for (int i = 0; i < matches.Count; i++) {
					...
					if (matches.Count > m_ImagesPool.Count) {
						...
						go.layer = ((Component) this).gameObject.layer;
				...
				for (int i = m_ImagesPool.Count - 1; i > 0; i--) {
					if (m_ImagesPool[i]) {
						if (!culled_ImagesPool.Contains(((Component) m_ImagesPool[i]).gameObject)) {
							culled_ImagesPool.Add(((Component) m_ImagesPool[i]).gameObject);
